...
object TimSort {
    ...
    private fun binarySort(a: IntBuffer, lo: Int, hi: Int, start_: Int, c: Comparator<Int>) {
        ...
        while (start < hi) {
            ...
            when (n) {
                ...
                1 -> a[left + 1] = a[left]
                else -> MemoryUtil.memCopy(MemoryUtil.memAddress(a, left), MemoryUtil.memAddress(a, left + 1), n.toLong() * Int.SIZE_BYTES)
            }
            a[left] = pivot
            start++
        }
    ...
    private fun mergeLo(base1: Int, len1_: Int, base2: Int, len2_: Int) {
        ...
        MemoryUtil.memCopy(MemoryUtil.memAddress(a, base1), MemoryUtil.memAddress(tmp, cursor1), len1.toLong() * Int.SIZE_BYTES)

        // Move first element of second run and deal with degenerate cases
        a[dest++] = a[cursor2++]
        if (--len2 == 0) {
            MemoryUtil.memCopy(MemoryUtil.memAddress(tmp, cursor1), MemoryUtil.memAddress(a, dest), len1.toLong() * Int.SIZE_BYTES)
            return
        }
        if (len1 == 1) {
            MemoryUtil.memCopy(MemoryUtil.memAddress(a, cursor2), MemoryUtil.memAddress(a, dest), len2.toLong() * Int.SIZE_BYTES)
        ...
        outer@ while (true) {
            ...
            do {
                ...
                if (count1 != 0) {
                    MemoryUtil.memCopy(MemoryUtil.memAddress(tmp, cursor1), MemoryUtil.memAddress(a, dest), count1.toLong() * Int.SIZE_BYTES)
                    dest += count1
                    cursor1 += count1
                    len1 -= count1
                ...
                if (count2 != 0) {
                    MemoryUtil.memCopy(MemoryUtil.memAddress(a, cursor2), MemoryUtil.memAddress(a, dest), count2.toLong() * Int.SIZE_BYTES)
                    dest += count2
                    cursor2 += count2
                    len2 -= count2
                    if (len2 == 0)
    ...
    private fun mergeHi(base1: Int, len1_: Int, base2: Int, len2_: Int) {
        ...
        MemoryUtil.memCopy(MemoryUtil.memAddress(a, base2), MemoryUtil.memAddress(tmp, tmpBase), len2.toLong() * Int.SIZE_BYTES)
        ...
        a[dest--] = a[cursor1--]
        if (--len1 == 0) {
            MemoryUtil.memCopy(MemoryUtil.memAddress(tmp, tmpBase), MemoryUtil.memAddress(a, dest - (len2 - 1)), len2.toLong() * Int.SIZE_BYTES)
            return
        }
        if (len2 == 1) {
            dest -= len1
            cursor1 -= len1
            MemoryUtil.memCopy(MemoryUtil.memAddress(a, cursor1 + 1), MemoryUtil.memAddress(a, dest + 1), len1.toLong() * Int.SIZE_BYTES)
            a[dest] = tmp[cursor2]
            return
        }
        ...
        outer@ while (true) {
            ...
            do {
                ...
                if (count1 != 0) {
                    ...
                    cursor1 -= count1
                    len1 -= count1
                    MemoryUtil.memCopy(MemoryUtil.memAddress(a, cursor1 + 1), MemoryUtil.memAddress(a, dest + 1), count1.toLong() * Int.SIZE_BYTES)
                    if (len1 == 0)
                        break@outer
                }
                a[dest--] = tmp[cursor2--]
                ...
                if (count2 != 0) {
                    ...
                    cursor2 -= count2
                    len2 -= count2
                    MemoryUtil.memCopy(MemoryUtil.memAddress(tmp, cursor2 + 1), MemoryUtil.memAddress(a, dest + 1), count2.toLong() * Int.SIZE_BYTES)
