...
inline fun <R : Comparable<R>> IntBuffer.sortBy(crossinline selector: (Int) -> R?) {
    if (lim > 1) sortWith(compareBy(selector))
}

inline fun <R : Comparable<R>> IntBuffer.sortByDescending(crossinline selector: (Int) -> R?) {
    if (lim > 1) sortWith(compareByDescending(selector))
}
...
infix fun IntBuffer.contentEquals(other: IntBuffer?): Boolean {
    ...
    val length = lim
    if (other.lim != length) return false
...
fun IntBuffer.toBooleanArray() = BooleanArray(lim) { this[it] != 0 }
fun IntBuffer.toByteArray() = ByteArray(lim) { this[it].toByte() }
fun IntBuffer.toCharArray() = CharArray(lim) { this[it].toChar() }
fun IntBuffer.toDoubleArray() = DoubleArray(lim) { this[it].toDouble() }
fun IntBuffer.toFloatArray() = FloatArray(lim) { this[it].toFloat() }


fun IntBuffer.toLongArray() = LongArray(lim) { this[it].toLong() }
fun IntBuffer.toShortArray() = ShortArray(lim) { this[it].toShort() }
// TODO unsigned?

inline fun <K, V> IntBuffer.associate(transform: (Int) -> Pair<K, V>): Map<K, V> {
    val capacity = maps.mapCapacity(lim).coerceAtLeast(16)
...
fun IntBuffer.copyOf(): IntBuffer {
    ...
    memCopy(memAddress(this), memAddress(dst), lim.toLong())
    return dst
}

fun IntBuffer.copyOf(newSize: Int): IntBuffer {
    val dst = memAllocInt(newSize)
    memCopy(memAddress(this), memAddress(dst), newSize.toLong())
    return dst
}
...
fun IntBuffer.sort() {
    if (lim > 1) _intArrays.sort(this)
}

fun IntBuffer.sortWith(comparator: Comparator<Int>) {
    if (lim > 1) _intArrays.sort(this, comparator)
}


fun IntBuffer.binarySearch(element: Int, fromIndex: Int = 0, toIndex: Int = lim) = _intArrays.binarySearch(this, fromIndex, toIndex, element)


fun IntBuffer.sort(fromIndex: Int = 0, toIndex: Int = lim) = _intArrays.sort(this, fromIndex, toIndex)

fun IntBuffer.sortWith(comparator: Comparator<Int>, fromIndex: Int = 0, toIndex: Int = lim) = _intArrays.sort(this, fromIndex, toIndex, comparator)

object _intArrays {

    fun sort(intBuffer: IntBuffer) = DualPivotQuicksort.sort(intBuffer, 0, intBuffer.lim - 1, null, 0, 0)

    fun sort(a: IntBuffer, c: Comparator<Int>?) {
        if (c == null) sort(a)
        else TimSort.sort(a, 0, a.lim, c, null, 0, 0)
    }

    fun binarySearch(a: IntBuffer, fromIndex: Int, toIndex: Int, key: Int): Int {
        rangeCheck(a.lim, fromIndex, toIndex)
