...
////    } as T
////
////    inline fun <R> readByte(block: (Adr) -> R): Byte = with { Ptr2<Byte>().apply { block(adr) }() }
////    inline fun <R> readInt(block: (Adr) -> R): Int = with { Ptr2<Int>().apply { block(adr) }() }
////
////    inline fun <R> readByte(block: (ByteBuffer) -> R): Byte = with {
////        val buf = it.malloc(1)
////        block(buf)
////        buf[0]
////    }
...
////    }
////
////    inline fun <R> pointerAdr(block: (Adr) -> R): Ptr<Any?> = with {
////        val pointer = it.mPointer()
////        block(pointer.adr)
////        pointer()
////    }
////
////    inline fun <R> pointerBuffer(block: (PointerBuffer) -> R): Ptr<Any?> = with {
...
//}
//
///** Same as [stack.invoke] but with a callsInPlace contract and with MemoryStack as receiver */
//
//inline fun <R> stac1k.with(block: MemoryStack.() -> R): R {
